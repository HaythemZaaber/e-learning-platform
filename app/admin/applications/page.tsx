            ...
            <div className="flex-1 min-w-0">
              ...
              {application.aiVerification && (
                <div className="flex items-center text-sm text-slate-600">
                  ...
                  <span>AI Identity Score: </span>
                  <span
                    className={`ml-1 font-medium ${
                      Math.round(
                        (application.aiVerification.overallScore || 0) * 100
                      ) >= 80
                        ? "text-emerald-600"
                        : Math.round(
                            (application.aiVerification.overallScore || 0) * 100
...
export default function ApplicationsList() {
      ...
      switch (activeTab) {
        ...
        case "info":
          filtered = filtered.filter(
            (app) => app.status === "REQUIRES_MORE_INFO"
          );
          break;
      }
    }
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      showToast(
        ...
        "Applications list has been updated successfully."
      );
    } catch (error) {
      ...
      showToast(
        "error",
        "Refresh Failed",
        "Failed to refresh applications list."
      );
    } finally {
  ...
  const getTabCount = (tab: string) => {
    switch (tab) {
      ...
      case "info":
        return applications.filter((app) => app.status === "REQUIRES_MORE_INFO")
          .length;
      default:
        return 0;
    }
