...
import { GET_ADMIN_STATS, GET_SUBMITTED_APPLICATIONS } from '@/graphql/queries/admin';
...
const ApplicationCard = ({ application, onViewClick }: { application: Application; onViewClick: (id: string) => void }) => {
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {application.status === 'REQUIRES_MORE_INFO' && application.manualReview && application.manualReview.conditionalRequirements && application.manualReview.conditionalRequirements.length > 0 && (
                <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  <div className="flex items-start space-x-2">
                    <AlertTriangle className="w-4 h-4 text-orange-600 mt-0.5 flex-shrink-0" />
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-orange-800 mb-2">Required Information</h4>
                      <div className="space-y-1">
                        {application.manualReview.conditionalRequirements.slice(0, 2).map((requirement: string, index: number) => (
                          <div key={index} className="flex items-start">
                            ...
                            <span className="text-xs text-orange-700">{requirement}</span>
                          </div>
                        ))}
            ...
            <span className={`${urgencyLevel === 'high' ? 'text-red-600 font-medium' : ''}`}>
              {daysSinceSubmission} days ago
                ...
                <div className="flex items-center">
                  ...
                  <span>{Object.keys(application.documents).length} documents</span>
                </div>
              )}
        </div>
      </CardContent>
...
export default function AdminDashboard() {
  ...
  const { data: applicationsData, loading: applicationsLoading, refetch: refetchApplications, error: applicationsError } = useQuery(GET_SUBMITTED_APPLICATIONS, {
    variables: {
      filters: {
        status: activeTab === 'pending' ? 'SUBMITTED' : 
  ...
  const allApplications: Application[] = Array.isArray(applicationsData?.getSubmittedApplications) 
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <h3 className="text-lg font-medium text-slate-900">Failed to load dashboard stats</h3>
          <p className="text-slate-600">
            {statsError.message.includes('fetch') || statsError.message.includes('network') 
  ...
  if (applicationsError) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center space-y-4">
          <AlertCircle className="w-12 h-12 text-red-500 mx-auto" />
          <h3 className="text-lg font-medium text-slate-900">Failed to load applications</h3>
          <p className="text-slate-600">
            {applicationsError.message.includes('fetch') || applicationsError.message.includes('network') 
              ? 'Unable to connect to the server. Please check your connection and try again.'
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-600 mt-1">Manage instructor applications and reviews</p>
              <p className="text-xs text-slate-500 mt-1">
                Last updated: {lastUpdated.toLocaleTimeString()}
              </p>
            </div>
            <Button onClick={handleRefresh} disabled={isRefreshing || applicationsLoading} variant="outline">
              <RefreshCw className={`w-4 h-4 mr-2 ${isRefreshing || applicationsLoading ? 'animate-spin' : ''}`} />
              ...
              <div className="p-6">
                {(applicationsLoading || isRefreshing) ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {!hasMoreApplications && filteredApplications.length > 0 && (
                      <div className="flex justify-center pt-6">
                        <Link href="/admin/applications">
                          <Button variant="outline" size="lg" className="group">
                            <span>View All Applications</span>
