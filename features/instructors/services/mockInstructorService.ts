import { InstructorProfile, InstructorHeroStats, TransformedInstructor } from '@/types/instructorGraphQLTypes';
...
const mockInstructors: InstructorProfile[] = [
  {
    ...
    expertise: ["Python", "Machine Learning", "Statistical Analysis", "Deep Learning"],
    qualifications: ["Ph.D. in Computer Science, Stanford University", "M.S. in Statistics, MIT"],
    experience: 10,
    socialLinks: {},
  ...
  },
  {
    ...
    qualifications: ["MFA in Design, Rhode Island School of Design", "B.A. in Graphic Design, Parsons School of Design"],
    experience: 15,
    socialLinks: {},
    ...
    marketingConsent: true,
    featuredInstructor: true,
    badgesEarned: ["Design Expert", "Verified Instructor", "Industry Professional"],
    lastCourseUpdate: new Date(),
    lastStudentReply: new Date(),
    lastContentCreation: new Date(),
    createdAt: new Date(),
  ...
  },
  {
    ...
    qualifications: ["B.S. in Computer Science, University of Texas", "Full Stack Web Development Bootcamp, Coding Dojo"],
    experience: 8,
    socialLinks: {},
    personalWebsite: "https://mariarodriguez.dev",
    linkedinProfile: "https://linkedin.com/in/mariarodriguez",
    subjectsTeaching: ["JavaScript", "React", "Node.js", "Full Stack Development"],
    ...
    marketingConsent: true,
    featuredInstructor: true,
    badgesEarned: ["Web Development Expert", "Verified Instructor", "Course Creator"],
    lastCourseUpdate: new Date(),
    lastStudentReply: new Date(),
    lastContentCreation: new Date(),
    createdAt: new Date(),
...
export const getMockFeaturedInstructors = (limit: number = 6): InstructorProfile[] => {
...
export const getMockInstructorsList = (
...
): { instructors: InstructorProfile[]; total: number; page: number; limit: number; totalPages: number; hasMore: boolean } => {
  ...
  if (filters?.categories?.length > 0) {
    filtered = filtered.filter(instructor =>
      instructor.teachingCategories.some(cat => filters.categories.includes(cat))
    );
  }
  
  if (filters?.minRating) {
    filtered = filtered.filter(instructor => (instructor.teachingRating || 0) >= filters.minRating);
  }
  
  // Apply sorting
  switch (sortBy) {
    case 'rating':
      filtered.sort((a, b) => (b.teachingRating || 0) - (a.teachingRating || 0));
      break;
    case 'students':
...
export const getMockAvailableTodayInstructors = (limit: number = 10): InstructorProfile[] => {
  return mockInstructors.slice(0, limit);
};
